...
const styleDate = function(date) {
  ...
  const arabicMonths = [
  "كانون الثاني", "شباط", "أذار", "نيسان", "أيار", "حزيران",
  "تموز", "أب", "أيلول", "تشرين الأول", "تشرين الثاني", "كانون الأول"
];
...
const createQuestions = async function(clicked) {
  ...
  // Highlight video
  [...document.querySelectorAll('.video-item')].forEach(vi => vi.classList.remove('active'))
  ...
  const questionsToLoad = await ajaxCall(`${domain}/api/v1/videos/${clicked.dataset.videoid}/questions`, 'POST', {subcourseId});
...
const handleQuestionSectionClick = async function(e) {
  ...
  if(role === 'admin') {
    ...
    if(clicked1) {
      const questionId = clicked1?.closest('.question-card')?.dataset.questionid;
  ...
  const questionToDelete = clicked2.closest('.question-card').dataset.questionid;
  ...
  if(clicked3){
    ...
    [...answerContainer.querySelectorAll('.answer-option')].forEach(cb => cb.classList.remove('selected'));
...
uploadQuestionBtn.addEventListener('click', function(e) {
  const videoId = [...document.querySelectorAll('.video-item')]?.find(vi => vi.classList.contains('active'))?.dataset?.videoid;
  if(!videoId) return;
...
videoContainer.forEach(btnsContainer => {
  ...
  const clicked1 = e.target.closest('.bi-edit')
  if(clicked1){
    return location.assign(`/lessons/${lessonId}/edit-video/${clicked1.closest('.video-item').dataset.videoid}`)
  }
...
const highlightAnswers = function(results) {
  ...
  results.forEach(result => {
    ...
    if(result.solvedRight == true) {
      ...
      const correctCheckBox = answers.find(answer => Number(answer.dataset.answernum) + 1 == result.correctAnswer);
...
const handleSubmit = async function() {
    const solvedQuestions = questions.map(q => {
      ...
      if(checked) answer = Number(checked.closest('.answer-option').dataset.answernum) + 1;
