...
const showError = function (field, msg) {
  ...
  const errorMsg = msg.startsWith("Validation Error")
    ? msg.split(":")[1].split(",")[0]
    : msg;
...
btnsList.addEventListener("click", (e) => {
  ...
  if (window.innerWidth < 500) list.classList.add("hidden");
});
...
nameEmailUpdateBtn.addEventListener("click", async (e) => {
  ...
  const data = await ajaxCall(`${domain}/api/v1/users/update-email-name`, {
    name,
    email,
  });
...
passwordUpdateBtn.addEventListener("click", async (e) => {
  ...
  if (!currentPassword)
    return showError("current-password", "الرجاء ادخال كلمة السر الحالية");
  if (!password) return showError("password", "الرجاء ادخال كلمة السر الجديدة");
  if (!passwordConfirm)
    return showError("password-confirm", "الرجاء تاكيد كلمة السر");
